...
class DashboardScreen2 extends StatefulWidget {
  final int index;
  const DashboardScreen2({Key? key, required this.index}) : super(key: key);
...
class _DashboardScreen2State extends State<DashboardScreen2> {
  ...
  late Future<List<Box>> screen2BoxesFuture;

  @override
  void initState() {
    super.initState();
    fetchUserInfo();
    initializeScreen2Boxes();
  }
  ...
  void initializeScreen2Boxes() {
    screen2BoxesFuture = Future.value([]);
  }
  ...
  Future<UserNew?> fetchUser() async {
    ...
    if (user == null) {
      ...
      List<Box> boxes = user.getScreen2BoxSetting();
      setState(() {
        screen2BoxesFuture = Future.value(boxes);
      });
    }
    return user;
  }
            ...
            return FutureBuilder<List<Box>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  final screen2Boxes = snapshot.data!;
