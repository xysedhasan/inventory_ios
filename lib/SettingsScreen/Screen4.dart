...
class Screen4 extends StatefulWidget {
  ...
  const Screen4({Key? key, required this.index}) : super(key: key);
...
class _Screen4State extends State<Screen4> {
  ...
  Future<String> getcustomImage() async {
    return Preferences.loadStoredValue("empty_box_custom_imageS4");
  }

  Future<int> getTotalBoxesS4() async {
    return Preferences.loadStoredIntValue("total_boxesS4");
  }

  Future<String> getEmptyboxS4() {
    return Preferences.loadStoredValue("empty_boxS4");
  }
  ...
  Future<void> _uploadImage() async {
    ...
    uploadTask.snapshotEvents.listen((snapshot) async {
      if (snapshot.state == TaskState.success) {
        ...
        await documentRef.update({'screen4.empty_box_custom_image': imageUrl});
        ...
        Preferences.saveToPreferences("empty_box_custom_imageS4", imageUrl);
      }
    });
  ...
  Future<void> _saveSettings() async {
    ...
    final DocumentReference documentRef =
    firestore.collection('users').doc(userId);
    await documentRef.update({'screen4.empty_box': getradiolotterybox()});
    await documentRef.update({'screen4.total_boxes': _lotteryTicketsToShow});
