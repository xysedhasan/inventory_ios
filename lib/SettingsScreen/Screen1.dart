...
class Screen1 extends StatefulWidget {
  ...
  const Screen1({Key? key, required this.index}) : super(key: key);
...
class _Screen1State extends State<Screen1> {
  ...
  Future<String> getcustomImage() async {
    return Preferences.loadStoredValue("empty_box_custom_imageS1");
  }

  Future<int> getTotalBoxesS1() async {
    return Preferences.loadStoredIntValue("total_boxesS1");
  }

  Future<String> getEmptyboxS1() {
    return Preferences.loadStoredValue("empty_boxS1");
  }
  ...
  Future<void> _uploadImage() async {
    ...
    uploadTask.snapshotEvents.listen((snapshot) async {
      if (snapshot.state == TaskState.success) {
        ...
        await documentRef.update({'screen1.empty_box_custom_image': imageUrl});
        ...
        Preferences.saveToPreferences("empty_box_custom_imageS1", imageUrl);
      }
    });
  ...
  Future<void> _saveSettings() async {
    ...
    getradiolotterybox();
    _lotteryTicketsToShow;
    print("valuesofradiobuttons");
    print(getradiolotterybox());
    print(_lotteryTicketsToShow);
    String userId = await Preferences.loadStoredValue("id");
    final FirebaseFirestore firestore = FirebaseFirestore.instance;
    final DocumentReference documentRef =
    firestore.collection('users').doc(userId);
    await documentRef.update({'screen1.empty_box': getradiolotterybox()});
    await documentRef.update({'screen1.total_boxes': _lotteryTicketsToShow});
