...
class Screen3 extends StatefulWidget {
  ...
  const Screen3({Key? key, required this.index}) : super(key: key);
...
class _Screen3State extends State<Screen3> {
  ...
  Future<String> getcustomImage() async {
    return Preferences.loadStoredValue("empty_box_custom_imageS3");
  }

  Future<int> getTotalBoxesS3() async {
    return Preferences.loadStoredIntValue("total_boxesS3");
  }

  Future<String> getEmptyboxS3() {
    return Preferences.loadStoredValue("empty_boxS3");
  }
  ...
  Future<void> _uploadImage() async {
    ...
    uploadTask.snapshotEvents.listen((snapshot) async {
      if (snapshot.state == TaskState.success) {
        ...
        await documentRef.update({'screen3.empty_box_custom_image': imageUrl});
        ...
        Preferences.saveToPreferences("empty_box_custom_imageS3", imageUrl);
      }
    });
  ...
  Future<void> _saveSettings() async {
    ...
    final DocumentReference documentRef =
    firestore.collection('users').doc(userId);
    await documentRef.update({'screen3.empty_box': getradiolotterybox()});
    await documentRef.update({'screen3.total_boxes': _lotteryTicketsToShow});
